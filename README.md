...
connections to the `gorao`. There are many ways to re-route traffic, but most
...
* Using homebrew:
    ```
    brew install ameshkov/tap/gorao
    ```
* From source:
    ```
    go install github.com/zamibd/gorao
    ```
* You can get a binary for your platform from
  the [releases page](https://github.com/zamibd/gorao/releases).

## Using `gorao`

### Redirect all traffic to a SNI proxy

* Run `gorao` and rewrite DNS responses to point to `1.2.3.4`:
  ```shell
  sudo gorao --dns-redirect-ipv4-to=1.2.3.4
  ```
...
Run `gorao`, rewrite DNS responses to point to `1.2.3.4`, :
...
Application Options:
      ...
      --dns-upstream=         The address of the DNS server the proxy will forward queries that are not
                              rewritten by gorao. (default: 8.8.8.8)
...
If you want to contribute to `gorao`, here are some tips on how to debug it
locally.

First, you rarely want to run with `sudo` and instead you'd prefer to use high
ports. `gorao` provides command-line arguments for that.
...
It is easy to use curl to debug `gorao`:
...
Use `mitmproxy` to debug `gorao` with forwarding rules:
