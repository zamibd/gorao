...
func New(cfg *Config) (d *DNSProxy, err error) {
	...
	d.proxy, err = proxy.New(&proxyConfig)
	if err != nil {
		return nil, fmt.Errorf("dnsproxy: cannot create proxy: %w", err)
	}
...
func (d *DNSProxy) Start() (err error) {
	...
	err = d.proxy.Start(context.Background())
...
func (d *DNSProxy) Close() (err error) {
	...
	err = d.proxy.Shutdown(context.Background())
...
func createProxyConfig(cfg *Config) (proxyConfig proxy.Config, err error) {
	...
	if cfg.TLSListenAddr.IsValid() {
		tcpAddr := &net.TCPAddr{
			IP:   net.IP(cfg.TLSListenAddr.Addr().AsSlice()),
			Port: int(cfg.TLSListenAddr.Port()),
		}
		proxyConfig.TLSListenAddr = []*net.TCPAddr{tcpAddr}
	}

	if cfg.HTTPSListenAddr.IsValid() {
		tcpAddr := &net.TCPAddr{
			IP:   net.IP(cfg.HTTPSListenAddr.Addr().AsSlice()),
			Port: int(cfg.HTTPSListenAddr.Port()),
		}
		proxyConfig.HTTPSListenAddr = []*net.TCPAddr{tcpAddr}
	}

	if cfg.QUICListenAddr.IsValid() {
		udpAddr := &net.UDPAddr{
			IP:   net.IP(cfg.QUICListenAddr.Addr().AsSlice()),
			Port: int(cfg.QUICListenAddr.Port()),
		}
		proxyConfig.QUICListenAddr = []*net.UDPAddr{udpAddr}
	}

	if cfg.TLSCertFile != "" && cfg.TLSKeyFile != "" {
		cert, err := tls.LoadX509KeyPair(cfg.TLSCertFile, cfg.TLSKeyFile)
		if err != nil {
			return proxyConfig, fmt.Errorf("failed to load TLS config: %w", err)
		}
	...
	proxyConfig.Logger = slog.Default()

	// Configure caching if enabled
	if cfg.CacheEnabled {
		proxyConfig.CacheEnabled = true
		proxyConfig.CacheSizeBytes = cfg.CacheSizeBytes
		proxyConfig.CacheMinTTL = cfg.CacheMinTTL
		proxyConfig.CacheMaxTTL = cfg.CacheMaxTTL
	}
