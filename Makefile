...
build: clean
	go build -ldflags "-X github.com/zamibd/gorao/internal/version.VersionString=$(VERSION)"

release: check-env-release
	mkdir -p $(BUILDDIR)
	cp README.md $(BUILDDIR)/
	CGO_ENABLED=0 GOOS=$(GOOS) GOARCH=$(GOARCH) go build -ldflags "-X github.com/zamibd/gorao/internal/version.VersionString=$(VERSION)" -o $(BUILDDIR)/$(NAME)$(ext)
